...
- 동영상 렌더 루프가 소스 프레임 속도로만 그리는지(60Hz 로 되그리지 않는지), 일시정지 중에도 요청 시 다시 그리는지

> 마지막 두 항목은 마지막 모니터에 테스트 패턴과 동영상을 **약 5초간 표시**한 뒤 닫습니다.
> (동영상 항목은 샘플 파일 인자를 넘겼을 때만 실행됩니다.)
...
|---|---|
| 계정 | `MinaryHub` (조직) |
| 페이지 | https://github.com/sponsors/minaryhub |
...
> 보는 사람이 조직으로도 후원할 수 있을 때만(=조직 관리자) 나타나며, 개인 계정만 있는 후원자에게는 표시되지 않습니다.
...
> 전부 PNG 로 넣으면 일부 도구에서 아이콘이 깨집니다.

---

## 구현 노트 (설계 결정)

- **vsync Present 는 렌더 락 밖에서 합니다.** `WarpRenderer.Render` 는 그리기만 하고 Present 는 호출자가
  락을 놓은 뒤에 합니다. 락을 쥔 채로 하면 vsync 대기(최대 약 16ms) 동안 캡처 스레드(WGC 콜백)와
  UI 스레드가 함께 멈춥니다.
- **동영상 루프는 새 프레임이나 변경이 있을 때만 그립니다.** 소스는 보통 24~30fps 인데 출력은 60Hz 라,
  매 바퀴 그리면 절반 이상이 같은 그림입니다. `_renderRequested` 플래그로 설정·오버레이 변경을 받아
  일시정지 중에도 편집 화면이 갱신되도록 합니다(`RequestRender` 가 이 플래그를 세웁니다).
- **호모그래피는 프레임당 한 번만 만듭니다.** 8x8 가우스 소거라 점마다 만들면 6x6 격자에서 프레임당
  150회를 넘습니다. 오버레이 기하와 메시 생성 모두 루프 밖에서 한 번만 만들어 넘깁니다.
- **번스타인 기저는 격자선마다 한 번만 구합니다.** `u` 는 열, `v` 는 행에만 의존하므로 정점마다 다시
  계산할 필요가 없습니다(`Bezier.Combine`).
